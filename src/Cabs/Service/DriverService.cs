...
public class DriverService : IDriverService
{
    ...
    public async Task ChangeDriverStatus(long? driverId, Driver.Statuses status)
    {
        ...
        if (status == Driver.Statuses.Active)
        {
            try
            {
                DriverLicense.withLicense(driver.DriverLicense.License);
            }
            catch (ArgumentException ex)
            {
                throw new InvalidOperationException($"Status cannot be ACTIVE.Illegal license no = {driver.DriverLicense.License}");
            }
        }
